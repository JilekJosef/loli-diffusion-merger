...
from modules import sd_models, sd_vae, scripts, devices
...
TYPESEG = ["none", "alpha", "beta (if Triple or Twice is not selected,Twice automatically enable)", "alpha and beta",
           "seed",
           "mbw alpha", "mbw beta", "mbw alpha and beta", "model_A", "model_B", "model_C",
           "pinpoint blocks (alpha or beta must be selected for another axis)",
           "include blocks", "exclude blocks", "elemental", "add elemental", "pinpoint element",
           "effective elemental checker", "adjust", "pinpoint adjust (IN,OUT,OUT2,CONT,BRI,COL1,COL2,COL3)",
           "calcmode", "prompt", "random"]
TYPES = ["none", "alpha", "beta", "alpha and beta", "seed", "mbw alpha ", "mbw beta", "mbw alpha and beta",
         "model_A", "model_B", "model_C", "pinpoint blocks", "include blocks", "exclude blocks", "elemental",
         "add elemental", "pinpoint element",
         "effective", "adjust", "pinpoint adjust", "calcmode", "prompt", "random"]
MODES = ["Weight", "Add", "Triple", "Twice", "AND gate"]
SAVEMODES = ["overwrite"]
EXCLUDE_CHOICES = ["BASE", "IN00", "IN01", "IN02", "IN03", "IN04", "IN05", "IN06", "IN07", "IN08", "IN09", "IN10",
                   "IN11",
                   "M00", "OUT00", "OUT01", "OUT02", "OUT03", "OUT04", "OUT05", "OUT06", "OUT07", "OUT08", "OUT09",
                   "OUT10", "OUT11",
                   "Adjust", "VAE"]
...
def smergegen(weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, base_deviation, mode,
              calcmode, useblocks, custom_name, save_sets, bake_in_vae, opt_value):

    result, currentmodel, modelid, theta_0, metadata = smerge(
        weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, base_deviation, mode, calcmode,
    ...
    result = savemodel(theta_0, currentmodel, custom_name, save_sets, metadata)
...
# Main Merging Code
def do_and_gate(theta_0_a, theta_1_a, theta_2_a, deviation):
    # store original shape
    theta_0_a_shape = theta_0_a.shape

    # make them flat
    def to_flat_list(mut_pass, index, theta_x_a):
        out = theta_x_a.flatten().tolist()
        mut_pass[index] = out

    mut_pass = [torch.tensor([0]), torch.tensor([0]), torch.tensor([0])]
    t0 = threading.Thread(target=to_flat_list, args=(mut_pass, 0, theta_0_a))
    t1 = threading.Thread(target=to_flat_list, args=(mut_pass, 1, theta_1_a))
    t2 = threading.Thread(target=to_flat_list, args=(mut_pass, 2, theta_2_a))
    t0.start()
    t1.start()
    t2.start()
    ...
    theta_0_a_flat_list = mut_pass[0]
    theta_1_a_flat_list = mut_pass[1]
    theta_2_a_flat_list = mut_pass[2]

    def process(theta_0_a_flat_list, theta_1_a_flat_list, theta_2_a_flat_list, start, end):
        if end is None:
            end = len(theta_0_a_flat_list)
        for j in range(start, end):
            if not abs(theta_1_a_flat_list[j] - theta_2_a_flat_list[j]) > deviation:
                theta_0_a_flat_list[j] = (theta_1_a_flat_list[j] + theta_2_a_flat_list[j]) / 2

    num_splits = multiprocessing.cpu_count() * 2
    split_size = len(theta_0_a_flat_list) // num_splits
    threads = []
    for i in range(num_splits):
        ...
        # create the thread
        threads.append(
            threading.Thread(target=process, args=(theta_0_a_flat_list, theta_1_a_flat_list, theta_2_a_flat_list, start, end)))
    ...
    for i in range(len(theta_0_a_flat_list)):
        if not abs(theta_1_a_flat_list[i] - theta_2_a_flat_list[i]) > deviation:
            theta_0_a_flat_list[i] = (theta_1_a_flat_list[i] + theta_2_a_flat_list[i])/2

    # return to original shape
    theta_0_a_flat = torch.FloatTensor(theta_0_a_flat_list)
    theta_0_a = theta_0_a_flat.view(theta_0_a_shape)

    return theta_0_a


def smerge(weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, base_deviation, mode, calcmode,
           useblocks, custom_name, save_sets, bake_in_vae, opt_value, main=[False, False, False]):
    caster("merge start", hearm)
    global hear, mergedmodel, statistics
    ...
    mergedmodel = [weights_a, weights_b,
                   ...
                   base_alpha, base_beta, mode, useblocks, custom_name, save_sets, calcmode, opt_value].copy()
    ...
    if useblocks:
        ...
        if not (len(weights_a) == 25 or len(
                weights_a) == 19): return f"ERROR: weights alpha value must be 20 or 26.", *NON4
        if usebeta:
            ...
            if not (len(weights_b) == 25 or len(weights_b) == 19):
                return f"ERROR: weights beta value must be 20 or 26.", *NON4

    caster("model load start", hearm)
    printstart(model_a, model_b, model_c, base_alpha, base_beta, weights_a, weights_b, mode, useblocks, calcmode)

    theta_1 = load_model_weights_m(model_b, 2, cache_target).copy()
    ...
    if MODES[1] in mode:  # Add
        if calcmode == "trainDifference" or calcmode == "extract":
            theta_2 = load_model_weights_m(model_c, 3, cache_target).copy()
        else:
            theta_2 = load_model_weights_m(model_c, 3, cache_target).copy()
    ...
    if "tensor" in calcmode or "self" in calcmode:
        theta_t = load_model_weights_m(model_a, 1, cache_target).copy()
        theta_0 = {}
        for key in theta_t:
            theta_0[key] = theta_t[key].clone()
        del theta_t
    else:
        theta_0 = load_model_weights_m(model_a, 1, cache_target).copy()

    if MODES[2] in mode or MODES[3] in mode or MODES[4] in mode:  # Tripe or Twice
        theta_2 = load_model_weights_m(model_c, 3, cache_target).copy()
    else:
    ...
    for num, key in enumerate(tqdm(theta_0.keys(), desc="Stage 1/2") if not False else theta_0.keys()):
        ...
        if calcmode == "trainDifference" or calcmode == "extract":
            ...
            if usebeta and (key not in theta_2) and (not theta_2 == {}):
                continue
        ...
        if useblocks:
            if weight_index > 0:
                ...
                if usebeta:
                    ...
                    # if len(deep) > 0:
        #    current_alpha = elementals(key,weight_index,deep,num,current_alpha)
        ...
        if calcmode == "normal":
            ...
            if MODES[1] in mode:  # Add
                ...
                if uselerp and current_alpha + current_beta != 0:
                    theta_0_a = lerp(theta_0_a.to(torch.float32),
                                     lerp(theta_1[key].to(torch.float32), theta_2[key].to(torch.float32),
                                          current_beta / (current_alpha + current_beta)),
                                     current_alpha + current_beta).to(theta_0_a.dtype)
                else:
                    theta_0_a = (1 - current_alpha - current_beta) * theta_0_a + current_alpha * theta_1[
                ...
                if uselerp:
                    theta_0_a = torch.lerp(
                        torch.lerp(theta_0_a.to(torch.float32), theta_1[key].to(torch.float32), current_alpha),
                        theta_2[key].to(torch.float32), current_beta).to(theta_0_a.dtype)
                else:
                    theta_0_a = (1 - current_alpha) * theta_0_a + current_alpha * theta_1[key]
                    theta_0_a = (1 - current_beta) * theta_0_a + current_beta * theta_2[key]

            elif MODES[4] in mode:  # AND gate
                caster(f"{num}, {block}, {model_a}+({model_b} AND {model_c}),{key}", hear)
                theta_0_a = do_and_gate(theta_0_a, theta_1[key], theta_2[key], base_deviation)
                ...
                if current_alpha == 1:
                    ...
                    caster(f"{num}, {block}, {key}, {model_a}*{1 - current_alpha}+{model_b}*{current_alpha}", hear)
                    if uselerp:
                        theta_0_a = torch.lerp(theta_0_a.to(torch.float32), theta_1[key].to(torch.float32),
                                               current_alpha).to(theta_0_a.dtype)
                    else:
                        theta_0_a = (1 - current_alpha) * theta_0_a + current_alpha * theta_1[key]
            ...
            theta_0[key] = extract_super(theta_0[key], theta_1[key], theta_2[key], current_alpha, current_beta,
                                         opt_value)
    ...
    if calcmode == "smoothAdd MT":
        ...
        theta_0, theta_1 = multithread_smoothadd(key_and_alpha, theta_0, theta_1, threads, tasks_per_thread, hear)

    currentmodel = makemodelname(weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, useblocks,
                                 mode, calcmode)
    ...
    saveekeys(keyratio, modelid)

    caster(mergedmodel, False)
    if "Reset CLIP ids" in save_sets:
        resetclip(theta_0)

    if True:  # always set metadata. savemodel() will check save_sets later
        merge_recipe = {
            "type": "loli-diffusion-merger",
        ...
        for key in (tqdm(theta_0.keys(), desc="Stage 0/2")):
            ...
            if "model" in key and key in theta_1:
                ...
                magnitude_similarity = dot_product / (
                        torch.norm(theta_0[key].to(torch.float32)) * torch.norm(theta_1[key].to(torch.float32)))
...
def cosine(mode, key, sim, sims, current_alpha, theta_0, theta_1, num, block, uselerp):
    if "A" in mode:  # favors modelA's structure with details from B
        ...
        if "model" in key and key in theta_0:
            ...
            if uselerp:
                theta_0[key] = lerp(theta_1[key].to(torch.float32), theta_0[key].to(torch.float32), k).to(
                    theta_0[key].dtype)
            else:
...
def multithread_smoothadd(key_and_alpha, theta_0, theta_1, threads, tasks_per_thread, hear):
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [executor.submit(thread_callback, extract_and_remove(keys, int(tasks_per_thread))) for i in
                   range(total_threads)]
...
################################################
##### Elementals
#def elementals(key, weight_index, deep, num, current_alpha):
...
def makemodelname(weights_a, weights_b, model_a, model_b, model_c, alpha, beta, useblocks, mode, calc):
    ...
    if useblocks:
        if MODES[1] in mode:  # add
            ...
            currentmodel = f"{model_a} + ({model_b} AND {model_c}) ({str(round(alpha, 3))},{','.join(str(s) for s in weights_a)})"
        else:  # Weight
        ...
        if MODES[1] in mode:  # add
            ...
            currentmodel = f"{model_a} + ({model_b} AND {model_c}) x {str(round(alpha, 3))}"
        else:  # Weight
...
def rwmergelog(mergedname="", settings=[], id=0):
    ...
    with  open(filepath, 'r+') as f:
        ...
        if mergedname != "":
            ...
            text = str(mergeid) + "," + datetime.datetime.now().strftime('%Y.%m.%d %H.%M.%S.%f')[
                                        :-7] + "," + text + "\n"
            f.writelines(text)
            return mergeid
        try:
...
def saveekeys(keyratio, modelid):
    import csv
    path_root = scripts.basedir()
    dir_path = os.path.join(path_root, "extensions", "loli-diffusion-merger", "scripts", "data")
    ...
    for w in blocks:
        ...
        if "-" in w:
            ...
            if blockids.index(wt[1]) > blockids.index(wt[0]):
                flagger[blockids.index(wt[0]):blockids.index(wt[1]) + 1] = [changer] * (
                        blockids.index(wt[1]) - blockids.index(wt[0]) + 1)
            else:
                flagger[blockids.index(wt[1]):blockids.index(wt[0]) + 1] = [changer] * (
                        blockids.index(wt[0]) - blockids.index(wt[1]) + 1)
        else:
...
def printstart(model_a, model_b, model_c, base_alpha, base_beta, weights_a, weights_b, mode, useblocks, calcmode):
